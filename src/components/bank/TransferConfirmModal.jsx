...
export function TransferConfirmModal({ amount, content, isSubmitting = false, onCancel, onConfirm, recipient, remainingBalance, summaryContent }) {
  return (
    <div aria-modal="true" className="fixed inset-0 z-40 grid place-items-center overflow-y-auto bg-ink/45 p-4 backdrop-blur-sm sm:p-5" role="dialog">
      <Card className="max-h-[calc(100vh-2rem)] w-full max-w-md overflow-y-auto bg-cream-card text-center">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Button className="justify-center" disabled={isSubmitting} onClick={onConfirm}>{isSubmitting ? content.submittingLabel : content.confirmLabel}</Button>
